...
const ATTACK_EFFECT_SCENE = preload("res://ui/attack_effect.tscn")
...
func build_hand() -> void:
	...
	for card_data in current_hand:
		var instance_id = card_data.get("instance_id", -1)
		var card_ui = existing_map.get(instance_id)
		if is_instance_valid(card_ui) and card_ui.get_parent() == hand_container and not card_ui.is_dragging:
			card_ui.call_deferred("_sync_layout_state")
			card_ui.call_deferred("set_selected", instance_id == battle_manager.selected_card_instance_id)
