...
func _draw_triggered_state() -> void:
	var gray := Color(0.35, 0.35, 0.35, 0.5)
	var dark_gray := Color(0.25, 0.25, 0.25, 0.5)
	
	var stem_base := Vector2(0.0, 55.0 * S)
	var stem_top := Vector2(0.0, -25.0 * S)
	for i in range(8):
		var t := float(i) / 7.0
		var y := lerpf(stem_base.y, stem_top.y, t)
		draw_line(Vector2(0, y), Vector2(0, y - 10 * S), dark_gray, 3.0 * S, true)
	
	var bud_center := stem_top
	_draw_petal(bud_center + Vector2(-10.0 * S, 5.0 * S), bud_center + Vector2(-3.0 * S, -42.0 * S), 12.0 * S, gray)
	_draw_petal(bud_center + Vector2(10.0 * S, 5.0 * S), bud_center + Vector2(3.0 * S, -42.0 * S), 12.0 * S, gray)
	_draw_petal(bud_center + Vector2(0.0, 6.0 * S), bud_center + Vector2(0.0, -46.0 * S), 9.0 * S, dark_gray)
