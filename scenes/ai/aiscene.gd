...
func show_card_in_ui(card_dict: Dictionary) -> void:
	...
	if scene:
		...
		if card_ui.has_signal("card_pressed"):
			card_ui.card_pressed.connect(_on_card_collected)


func _on_card_collected(_card_index: int) -> void:
	_collect_card_and_continue()


func _collect_card_and_continue() -> void:
	for child in card_container.get_children():
		var tw := create_tween()
		tw.tween_property(child, "scale", Vector2(0.1, 0.1), 0.3)
		tw.parallel().tween_property(child, "modulate:a", 0.0, 0.3)
		await tw.finished
	
	card_container.hide()
	await get_tree().create_timer(0.3).timeout
	
	if typeof(Game) == TYPE_OBJECT and Game.has_method("goto_explore"):
		Game.goto_explore()


func play_card(_card_idx, _card_dict=null) -> void:
	_collect_card_and_continue()
